...
describe("integration/runs/redshift-table", () => {
  ...
  test(
    "a redshift schedule can run and create records",
    async () => {
      // enqueue the run
      session.params = {
        csrfToken,
      ...
      expect(foundSendTasks.length).toEqual(0);
  ...
  test(
    "a redshift schedule can run and update records only finding updated records",
    async () => {
      // enqueue the run
      session.params = {
        csrfToken,
