...
describe("integration/runs/clickhouse", () => {
  ...
  test("an administrator can create the related import app and schedule", async () => {
    ...
    session.params = {
      ...
      type: "clickhouse-export-records",
      appId: app.id,
      modelId: model.id,
      options: {
  ...
  test("we can read the clickhouse mapping options", async () => {
    session.params = {
      csrfToken,
      id: destination.id,
    };
  ...
  test(
    "a ClickHouse schedule can run and create records",
    async () => {
      // enqueue the run
      session.params = {
        csrfToken,
  ...
  test(
    "a ClickHouse schedule can run and update records only finding updated records",
    async () => {
      // enqueue the run
      session.params = {
        csrfToken,
