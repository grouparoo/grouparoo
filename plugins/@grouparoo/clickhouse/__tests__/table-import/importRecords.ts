...
describe("clickhouse/table/importRecords", () => {
  ...
  test("imports all records when there is a highWaterMark", async () => {
    let limit = 100;
    let highWaterMark = { stamp: "2020-02-07 12:13:14.000" };
    let scheduleFilters = [];
    let sourceOffset = 0;
    const { imports, importsCount } = await runIt({
      limit,
  ...
  test("handles getting no results", async () => {
    let limit = 100;
    let sourceOffset = 0;
    let highWaterMark = { stamp: "2020-02-11 12:13:14.000" }; // past the last one
    let scheduleFilters = [];
    const { imports, importsCount } = await runIt({
      limit,
      highWaterMark,
