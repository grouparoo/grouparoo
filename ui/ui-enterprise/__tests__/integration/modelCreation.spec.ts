...
test.describe("Model Creation", () => {
  ...
  test("creates team and signs in", async () => {
    const initializeTeam = new InitializeTeamPageObject(page);
    await initializeTeam.navigate();
    await initializeTeam.fillAndSubmit({
      companyName,
      firstName,
      lastName,
      email,
    ...
    const signIn = new SignInPageObject(page);
    await signIn.navigate();
    await signIn.fillAndSubmit({
      email,
      password,
    });
  });
  ...
  test("it runs all schedules after clicking button", async () => {
    await modelOverview.navigate();
    await modelOverview.clickRunAllSchedules();
  });
